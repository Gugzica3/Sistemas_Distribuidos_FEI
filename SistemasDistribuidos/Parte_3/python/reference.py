...
Responsibilities:
  ...
  5. Provide current physical timestamp for clock synchronisation (ref_time in all replies)
  6. Participate in Lamport logical clock exchange

All communication uses ZeroMQ REP socket + MessagePack serialisation.
"""
...
class LamportClock:
    ...
    def update(self, received: int) -> int:
        """Update on receive: clock = max(clock, received)."""
        with self._lock:
...
# ── Server registry ───────────────────────────────────────────────────────────
_servers: dict = {}          # name -> {"rank": int, "last_seen": float}
_rank_counter: int = 0
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"  Reference Service started")
    ...
    while True:
        ...
        try:
            ...
            err = {"type": "ERROR", "error": str(e),
                   "logical_clock": clock.tick(), "ref_time": now()}
            socket.send(pack(err))
            continue

        # Update logical clock from sender
